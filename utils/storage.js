...
const developerErrorsPathName = path.join(
  homedir,
  '.zt',
  'developerError.json'
);
...
const getStorageValues = (type = 'regular', decrypt = true) => {
  try {
    const rawdata = fs.readFileSync(
      type === 'regular' || type === 'eth' ? pathName : proxyData
    );
...
const createFolders = () => {
  const createFolder = (name) =>
    !fs.existsSync(name) && fs.mkdirSync(name, { recursive: true });
  createFolder(dir);
...
const updateHistoryValues = (values) =>
  updateValues(historyPathName, values, 'array');
const getCredentials = () => getValues(credentialPathName);
const setCredentials = (email, password, terms_conditions, passphrase) =>
  updateValues(credentialPathName, {
    email,
    password,
    terms_conditions,
    passphrase,
  });
const resetCredentials = (password) =>
  updateValues(credentialPathName, { password });
const getHolon = () => getValues(holonPathName);
const setHolon = (id, port, rating, complaints) =>
  updateValues(holonPathName, { id, port, rating, complaints });
const getLinkedinCookieValues = () => getValues(linkedinCookiePath);
const updateLinkedinCookieValues = (values) =>
  updateValues(linkedinCookiePath, values, 'array', false);
const getProxyHolonValues = () => getValues(proxyHolonPath);
const updateProxyHolonValues = (values) =>
  updateValues(proxyHolonPath, values, 'object', false);
...
const updateGeneralErrorValues = (values, reset) =>
  updateValues(generalErrorsPathName, values, 'array', !reset);
const getDeveloperErrorValues = () =>
  getValues(developerErrorsPathName, 'array');
const updateDeveloperErrorValues = (values, reset) =>
  updateValues(developerErrorsPathName, values, 'array', !reset);
const getUmbrellaPaths = () => getValues(umbrellaPath, 'object');
const updateUmbrellaPaths = (values) =>
  updateValues(umbrellaPath, values, 'object', false);
