...
function encrypt(text, passcode = 'ZeRoThefT123') {
  const key = crypto.createHash('sha256').update(String(passcode)).digest('base64').substr(0, 32)
...
function encryptAmount(amount) {
  const newAmount = amount.slice(1).slice(0, -1).split('')
...
function decryptAmount(amountStr) {
  const newAmountStr = amountStr.slice(2).toLowerCase().split('')
