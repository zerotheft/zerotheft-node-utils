...
const indexOfStatus = (status) => {
  return HOLONSTATUSES.indexOf(status)
}

const convertStringDollarToNumeric = (summary) => {
  try {
    let values = summary.match(/^\$\s?([\d]+|([\d]{1,3},(([\d]{3},)+)?[\d]{3}))(\.[\d]+)?\s?([KMBT])?$/)
...
const updateProposalYaml = async (yamlFile, newVal) => {
  return new Promise((resolve, reject) => {
    let doc = yaml.safeLoad(fs.readFileSync(yamlFile, 'utf8'))
    doc.proposal = newVal
...
const getDate = (date, dateFormat = 'dd MMM, yyyy') => {
