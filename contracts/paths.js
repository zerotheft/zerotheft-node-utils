...
const getHierarchyContractVersion = async (hierarchyContract = null) => {
  ...
  try {
    const versionNumber = await hierarchyContract.callSmartContractGetFunc(
      'getContractVersion'
    );
    return {
      success: true,
...
const listHierarchyAreas = async (contract = null) => {
  ...
  while (version > 0) {
    try {
      ...
      const areas = await contract.callSmartContractGetFunc(
        'getEconomicHierarchyAreas',
        [version]
      );
...
const fetchPathYaml = async (
  contract,
  yamlBlockHash,
  index,
  allOutputs = []
) => {
  const yamlBlock = await contract.callSmartContractGetFunc(
    'getEconomicHierarchyYamlBlock',
    [yamlBlockHash],
    900000
  );
...
const pathYamlContent = async (
  ...
  if (!fs.existsSync(pathDir)) {
    ...
    const outputFiles = await fetchPathYaml(
      pathContract,
      hierarchyYaml.firstBlock,
      1
    );
...
const pathsByNation = async (nation = 'USA', area = 'RiggedEconomy') => {
  ...
  const path = await contract.callSmartContractGetFunc(
    'getLatestEconomicHierarchy',
    [hierarchyAreaBytes]
  );
  let yamlContent;
  if (Object.keys(path).length > 0) {
    yamlContent = await pathYamlContent(
      contract,
...
const getPathDetail = async (
  ...
  try {
    ...
    while (verRes.number > 0) {
      const {
        propIds,
      } = await proposalContract.callSmartContractGetFunc(
        'allProposalsByPath',
        [convertStringToHash(path), verRes.number]
      );
      if (propIds.length === 0)
        throw new Error(`no proposals found for ${path}`);
      const { results: pathDetails } = await PromisePool.withConcurrency(10)
        ...
        .process(async (id) => {
          ...
          const { results: voteInfo } = await PromisePool.withConcurrency(10)
            ...
            .process(async (vid) => {
              try {
                const singleVoterInfo = await voterContract.callSmartContractGetFunc(
                  'getVote',
                  [vid]
                );
                ...
                return {
                  ...
                  altTheftAmt:
                    singleVoterInfo.customTheftAmount === ''
                      ? {}
                      : JSON.parse(singleVoterInfo.customTheftAmount),
                  path: path.split('/').slice(1).join('/'),
                  proposalId: id,
                  votedYears: Object.keys(proposal.theftYears).map((y) =>
                    parseInt(y)
                  ),
                };
              } catch (e) {
...
const nextVotingArea = async () => {
  ...
  for (const pn of Object.keys(priorityList)) {
    ...
    for (const hash of Object.keys(priorityAreas)) {
      ...
      if (proposalsInPath.length === 0) {
        continue;
      }

      const voteCount = areaVotes[hash] || 0;
      // Get the rank of area based on votes given
      const voteRange = votesRank.find(
        (p) => voteCount >= p.minVote && voteCount <= p.maxVote
      );
      const rank = voteRange ? voteRange.rank : defaultRank;
      // Calculate the score based on rank
      const score = (10 - priority) * rank;
      const scoreSheetItem = get(scores, score, { _areas: [] });
      const newArea = {
        hash,
