...
const fetchProposalYaml = async (
  ...
  const yamlBlock = await proposalContract.callSmartContractGetFunc(
    'getProposalYamlBlock',
    [yamlBlockKey]
  );
  ...
  if (yamlBlock.nextYamlBlock !== '' && !finalIndex) {
    await fetchProposalYaml(
      proposalContract,
      yamlBlock.nextYamlBlock,
      index + 1,
      allOutputs,
      initHash
    );
...
const getProposalContractVersion = async (proposalContract = null) => {
  ...
  try {
    const versionNumber = await proposalContract.callSmartContractGetFunc(
      'getContractVersion'
    );
    return {
      success: true,
...
const getProposalIDByIndex = async (proposalIndex, proposalContract = null) => {
  ...
  try {
    const contractVersion = await proposalContract.callSmartContractGetFunc(
      'getContractVersion'
    );
...
const listProposalIds = async (contract = null) => {
  ...
  while (verRes.number > 0) {
    ...
    try {
      do {
        const proposalIds = await contract.callSmartContractGetFunc(
          'getproposalIndicesByCursor',
          [cursor, howMany, verRes.number]
        );
...
const proposalRating = async (feedbackContract, proposalID) => {
  try {
    ...
    let allFeedbackers = [];
    while (version > 0) {
      const feedbackers = await feedbackContract.callSmartContractGetFunc(
        'totalProposalRaters',
        [proposalID, version]
      );
    ...
    const ratingPromises = allFeedbackers.map(async (feedbacker) => {
      const feedback = await feedbackContract.callSmartContractGetFunc(
        'getProposalRating',
        [proposalID, feedbacker]
      );
    ...
    return {
      success: true,
      count: allFeedbackers.length,
      rating: mean(allRatings) || 0,
    };
  } catch (e) {
    return { success: false, error: e.message };
  }
...
const proposalComplaints = async (feedbackContract, proposalID) => {
  try {
    ...
    let version = verRes.number;
    while (version > 0) {
      const complainers = await feedbackContract.callSmartContractGetFunc(
        'totalProposalCommentors',
        [proposalID, version]
      );
    ...
    const complaintPromises = allComplainers.map(async (complainer) => {
      const countRes = await feedbackContract.callSmartContractGetFunc(
        'countCitizenCommentsToProposal',
        [proposalID, complainer]
      );
      ...
      for (let i = 1; i <= parseInt(countRes); i++) {
        const complaintInfo = await feedbackContract.callSmartContractGetFunc(
          'getCitizenCommentToProposal',
          [proposalID, complainer, i]
        );
    ...
    return {
      success: true,
      count: Object.keys(complaints).length,
...
const getProposalDetails = async (proposalId, proposalContract = null) => {
  ...
  const proposal = await proposalContract.callSmartContractGetFunc(
    'getProposal',
    [proposalId]
  );
  const filePath = `${tmpPropDir}/main-${proposal.yamlBlock}.yaml`;
  if (Object.keys(proposal).length > 0) {
    const proposalYaml = await proposalContract.callSmartContractGetFunc(
      'getProposalYaml',
      [proposal.yamlBlock]
    );
  ...
  return {
    ...
    title:
      file && (file.title || file.Title)
        ...
        : 'No Title available',
    description:
      file && file.describe_problem_area
        ? file.describe_problem_area
...
const getProposals = async (proposalIDs, proposalContract = null) => {
  ...
  const promises = proposalIDs.map(async (proposalIndex) => {
    ...
    const proposalDetail = await proposalContract.callSmartContractGetFunc(
      'getProposal',
      [proposalID]
    );
...
const allProposals = async () => {
  ...
  proposalContract.init();
  const proposalIDs = await proposalContract.callSmartContractGetFunc(
    'getproposalIds',
    []
  );
...
const proposalsFromEvents = async (methodName, args = {}) => {
  const proposalContract = getProposalContract();
  return await proposalContract.watchEvent(
    'LogProposalCreation',
    methodName,
    args
  );
...
const getCachedProposalsByPathsDir = (path) => {
  const cachedProposalsByPathsDir = dir.join(
    homedir,
    '.cache',
    'proposals_by_paths'
  );
...
const proposalIdsByPath = async (pathHash, proposalContract = null) => {
  ...
  while (verRes.number > 0) {
    const {
      propIds,
    } = await proposalContract.callSmartContractGetFunc('allProposalsByPath', [
      pathHash,
      verRes.number,
    ]);
...
const getPathProposalsByPath = async (path, contract) => {
  ...
  const { data: cachedProposalsByPaths, file } = getCachedProposalsByPathsDir(
    pathHash
  );
  // get cachedproposal
  let cachedFiles = [];
    ...
    const { results, errors } = await PromisePool.withConcurrency(1)
      ...
      .process(async (pid) => {
        try {
          ...
          const pData = await getProposalData(
            pid,
            cachedProposalsByPaths,
            proposalContract,
            cachedFiles,
            path
          );
          ...
          return {
            ...pData.proposal,
            votes: !isEmpty(proposalVotes)
              ? get(proposalVotes, pid, []).length
              : 0,
            ratings,
            complaints,
          };
  ...
  if (!isEmpty(newProposals)) {
    fs.writeFileSync(
      file,
      JSON.stringify({ ...cachedProposalsByPaths, ...newProposals })
    );
  }
  return allPropsData;
};
 ...
 */
const getYamlFromCacheOrSmartContract = async (
  proposalId,
  path,
  contract,
  cachedYamls
  ...
  const proposal = await proposalC.callSmartContractGetFunc('getProposal', [
    proposalId,
  ]);
  ...
  try {
    ...
    throw new Error('no cached exported proposal file.');
  } catch (e) {
    // if not found in cache then search blockchain
    filePath = `${tmpPropDir}/main-${proposal.yamlBlock}.yaml`;

    if (Object.keys(proposal).length > 0) {
      const proposalYaml = await proposalC.callSmartContractGetFunc(
        'getProposalYaml',
        [proposal.yamlBlock]
      );
      const outputFiles = await fetchProposalYaml(
...
const getProposalData = async (
  ...
  const {
    ...
    yamlJSON: file,
  } = await getYamlFromCacheOrSmartContract(
    proposalId,
    path,
    proposalC,
    cachedYamls
...
const getProposalYaml = async (proposalId, path, contract) => {
  const { yamlJSON } = await getYamlFromCacheOrSmartContract(
    proposalId,
    path,
    contract
  );
